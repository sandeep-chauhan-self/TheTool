...
def fetch_all_nse_stocks_from_csv() -> List[Dict[str, str]]:
    ...
    Returns:
        List[Dict[str, str]]: List of dictionaries with symbol and company name
    """
    headers = {
    ...
    try:
        ...
        session.get('https://www.nseindia.com/', headers=headers, timeout=NSE_REQUEST_TIMEOUT_SHORT)
        time.sleep(NSE_REQUEST_DELAY)
        
        # Fetch the CSV
        response = session.get(NSE_ALL_EQUITY_URL, headers=headers, timeout=NSE_REQUEST_TIMEOUT_LONG)
...
def fetch_nse_stocks_from_api() -> List[Dict[str, str]]:
    """
    ...
    try:
        ...
        for index in indices:
            try:
                ...
                response = session.get(url, headers=headers, timeout=NSE_REQUEST_TIMEOUT_SHORT)
...
def save_to_csv(stocks: List[Dict[str, str]], output_file: str = 'data/nse_stocks_complete.csv') -> None:
    """
...
def save_stocks_to_files(stocks: List[Dict[str, str]]) -> bool:
    """
    Save stock symbols to multiple formats (TXT, JSON, CSV)
    
    Args:
        stocks: List of stock dictionaries with symbol and name
